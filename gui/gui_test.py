...
# New code
        ...
        if isinstance(self.src0, int):
            cap0 = PiCameraCapture(self.src0)
            cap1 = PiCameraCapture(self.src1)
        else:
...
class PoseViewer(QMainWindow):
    ...
    def __init__(self, src0: str, src1: str, meta_json: Optional[str] = None):
        ...
        # -- background worker ---------------------------------------------#
        print(f"src0={src0} type={type(src0)}, src1={src1} type={type(src1)}")
...
def main() -> None:
    ...
    # Allow passing camera indices or file paths as arguments:
    # python gui.py 0 1          <- two cameras
    # python gui.py a.mp4 b.mp4  <- two video files
    if len(sys.argv) == 3:
        src0, src1 = sys.argv[1], sys.argv[2]
        # Convert to int if numeric (camera index)
        try:
            src0 = int(src0)
        except ValueError:
            pass
